...
export const Gallery = () => {
  ...
  return (
    <section id="gallery" className="w-full min-h-screen bg-[#000103] relative overflow-hidden py-24">

      <div className="absolute top-0 right-0 w-96 h-96 bg-[#f98f0a]/8 rounded-full blur-[150px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-16">
        ...
        <div className="flex flex-col items-center text-center mb-16">
          ...
          <div className="flex flex-row items-center gap-4 mb-4">
            <motion.div className="h-px w-12 bg-[#f98f0a]" initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} transition={{ duration: 0.6, delay: 0.2 }} viewport={{ once: true }} />
            <motion.h2
              ...
              viewport={{ once: true }}
            >
              Галерея <span className="text-[#f98f0a]">робіт</span>
            </motion.h2>
            <motion.div className="h-px w-12 bg-[#f98f0a]" initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} transition={{ duration: 0.6, delay: 0.2 }} viewport={{ once: true }} />
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {images.map((img, i) => (
            <motion.div
              key={img.id}
              className="relative cursor-pointer overflow-hidden rounded-sm border border-white/10 hover:border-[#f98f0a]/50 transition-colors duration-500"
            ...
            >
              ...
              {/* Overlay */}
              <div className={`absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent transition-opacity duration-500 ${isAfterVisible(img.id) ? "opacity-100" : "opacity-0"}`} />
              ...
              <div className="absolute top-3 left-3 flex gap-2">
                ...
                <motion.span
                  className="bg-[#f98f0a] text-white text-xs font-semibold px-2 py-1 rounded-sm tracking-wider"
                  animate={{ opacity: isAfterVisible(img.id) ? 1 : 0 }}
                  transition={{ duration: 0.3 }}
                >
                  ПІСЛЯ
              ...
              {isMobile && (
                <motion.div
                  ...
                  animate={{ opacity: isAfterVisible(img.id) ? 1 : 0.5, y: isAfterVisible(img.id) ? 0 : 4 }}
                  transition={{ duration: 0.3 }}
                >
              ...
              {!isMobile && (
                <motion.div
                  ...
                  animate={{ y: isAfterVisible(img.id) ? 0 : 8, opacity: isAfterVisible(img.id) ? 1 : 0 }}
                  transition={{ duration: 0.3 }}
                >
                  <span>Збільшити</span>
                  <span>↗</span>
            ...
            >
              ...
              <div className={`grid gap-2 ${isMobile ? "grid-cols-1" : "grid-cols-2"}`}>
                <div className="relative">
                  <img src={images[selected].before} alt="До" className="w-full object-cover rounded-sm" />
                  <span className="absolute top-3 left-3 bg-black/70 text-white text-xs font-bold px-3 py-1 tracking-widest backdrop-blur-sm">ДО</span>
                </div>
                <div className="relative">
                  <img src={images[selected].after} alt="Після" className="w-full object-cover rounded-sm" />
                  <span className="absolute top-3 left-3 bg-[#f98f0a] text-white text-xs font-bold px-3 py-1 tracking-widest">ПІСЛЯ</span>
                </div>
              </div>

              <div className="flex justify-between mt-4">
                <button
                  className="text-white/60 hover:text-[#f98f0a] transition-colors text-sm tracking-widest uppercase cursor-pointer"
                  onClick={() => setSelected((prev) => (prev! > 0 ? prev! - 1 : images.length - 1))}
                >
                  ← Попередній
                </button>
                <span className="text-white/30 text-sm" style={{ fontFamily: "var(--font-display)" }}>
