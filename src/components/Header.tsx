...
export const Header = () => {
  ...
  return (
    // SEO: role="banner" — стандартна роль для header, допомагає пошуковикам
    <header
      role="banner"
      className={`w-screen flex flex-row items-center justify-between px-4 sm:px-8 py-2 transition-all duration-500 z-50 ${
        scrolled
          ? "bg-[var(--main-black)]/80 backdrop-blur-md shadow-lg shadow-black/20 border-b border-[var(--main-green-muted)]/10"
          : "bg-transparent"
      }`}
    >
      {/*
        SEO: alt логотипу — назва бізнесу + локація + послуга.
        Google використовує alt логотипу для підтвердження назви бренду.
      */}
      <Link to="hero" smooth={true} duration={500} aria-label="Auto PDR Master — на головну">
        <img
          src="images/main-logo.jpg"
          alt="Auto PDR Master — PDR сервіс у Білогородці"
      ...
      {isDesktop ? (
        // SEO: <nav> з aria-label — Google розуміє це як основну навігацію сайту
          ...
          <ul className="flex flex-row gap-6 xl:gap-8 justify-center items-center">
            {navItems.map((item, index) => (
              <Link to={item.id} smooth={true} duration={500} offset={-70} key={index}>
                <li
                  className="relative text-white/80 font-semibold tracking-wide cursor-pointer group 
        ...
        <div className="relative z-50">
          {/* SEO: aria-label на кнопці бургера — доступність і зрозумілість для Google */}
          <button
            onClick={() => setMenuOpen(!menuOpen)}
            className="flex flex-col gap-1.5 cursor-pointer p-2"
            aria-label={menuOpen ? "Закрити меню" : "Відкрити меню навігації"}
          ...
          {isMenuVisible && (
            // SEO: nav + aria-label для мобільного меню
