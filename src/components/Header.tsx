...
export const Header = () => {
  ...
  const navItems = ["Головна", "Про нас", "Послуги", "Контакти", "Галерея", "Замовлення"];
      ...
      {isDesktop ? (
        <ul className="flex flex-row gap-6 xl:gap-8 justify-center items-center">
          {navItems.map((item, index) => (
            <li key={index} className="relative text-white font-semibold tracking-wide cursor-pointer group transition-colors duration-300 hover:text-[#f98f0a] text-sm xl:text-base">
              {item}
